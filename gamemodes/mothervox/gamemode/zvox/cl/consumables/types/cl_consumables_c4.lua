...
local nextCanC4 = 0
ZVox.NewControlListener("item_c4", "c4_use", function()
	...
	local currC4 = ZVox.Consumable_GetCurrentCount(MV_CONSUMABLE_C4)
	if currC4 <= 0 then
		return
	end
	...
	if nextCanC4 > CurTime() then
		return
	end
	nextCanC4 = CurTime() + 1.25

	surface.PlaySound("mothervox/sfx/consumables/c4.wav")
	ZVox.DoScreenShake(64, 1.25)
	ZVox.AddMinedPopup("Used Plastic Explosives", colBlue)
	...
	ZVox.Consumable_SpendConsumable(MV_CONSUMABLE_C4)
	ZVox.PushExplosionFX(1)
end)