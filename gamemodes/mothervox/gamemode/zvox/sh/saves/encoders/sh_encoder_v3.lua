...
if ZVOX_ENCODER_MAXVERSION < 3 then
	ZVOX_ENCODER_MAXVERSION = 3
end
...
local function writeChunk(fBuff, chunk)
	local voxelData = ZVox.SerializeChunkVoxelData(chunk["voxelData"])
	ZVox.FB_WriteULong(fBuff, #voxelData)
	ZVox.FB_Write(fBuff, voxelData)
	voxelData = nil -- garbage collect

	-- voxelstate now
	local voxelState = ZVox.SerializeChunkVoxelData(chunk["voxelState"])
...
-- Encoder V3 a copy paste of encodeer v2
-- this shit is progressive so remember to return 
local CHUNK_ITERATIONS = 16 * 2
local function encodeV3(fPtr, univObj, persistData)
...
ENCODER["encodeFunc"] = encodeV3
ZVOX_ENCODER_LIST[3] = ENCODER